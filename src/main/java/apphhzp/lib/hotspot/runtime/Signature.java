...
public class Signature {
    ...
    static @RawCType("BasicType") int basic_type(Symbol signature) {
        return basic_type(signature.char_at(0));
    }

    // Returns T_ILLEGAL for an illegal signature char.
    static @RawCType("BasicType") int basic_type(int ch){
    ...
    static boolean not_class_name(Symbol signature) {
    ...
    static boolean is_array(Symbol signature) {
    ...
    static boolean is_valid_array_signature(Symbol sig) {
        if (sig.getLength()<=1){
    ...
    static boolean has_envelope(Symbol signature) {
    ...
    static boolean has_envelope(char signature_char) {
    ...
    static boolean is_method(Symbol signature) {
        return signature.char_at(0)=='(';
    }

    // Assuming it's a method signature, determine if it must
    // return void.
    static boolean is_void_method(Symbol signature) {
