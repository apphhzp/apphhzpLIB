 ...
 * and return value.*/
public class CallTypeData extends CounterData {
    // entries for arguments if any
    private TypeStackSlotEntries _args;
    // entry for return type if any
    private ReturnTypeEntry _ret;
    public CallTypeData(DataLayout layout) {
        super(layout);
        if (!(layout.tag() == DataLayout.call_type_data_tag)){
            throw new IllegalArgumentException("wrong type");
        }
        _args=new TypeStackSlotEntries(CounterData.static_cell_count()+TypeEntriesAtCall.header_cell_count(), number_of_arguments());
    ...
    public boolean is_CallTypeData(){ return true; }
