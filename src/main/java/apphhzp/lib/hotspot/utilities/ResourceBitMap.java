...
public class ResourceBitMap {
    public ResourceBitMap(int sizeInBits) {
    ...
    public boolean at(int offset) {
        if (JVM.ENABLE_EXTRA_CHECK){
            if (!(offset>=0 && offset < size())){
                throw new IndexOutOfBoundsException("BitMap index out of bounds");
            }
        }
        return (wordFor(offset)&JVM.nthBit(offset % bitsPerWord))!=0; //Bits.isSetNthBit(, );
    }
...
//    }
    ...
    public boolean setUnion(ResourceBitMap other) {
        if (JVM.ENABLE_EXTRA_CHECK) {
            if (!(size() == other.size())){
                throw new RuntimeException("must have same size");
            }
        }
    ...
    public void setIntersection(ResourceBitMap other) {
        if (JVM.ENABLE_EXTRA_CHECK) {
            if (!(size() == other.size())){
                throw new RuntimeException("must have same size");
            }
    ...
    public void setFrom(ResourceBitMap other) {
        if (JVM.ENABLE_EXTRA_CHECK) {
            if (!(size() == other.size())){
                throw new RuntimeException("must have same size");
            }
    ...
    public boolean setDifference(ResourceBitMap other) {
        if (JVM.ENABLE_EXTRA_CHECK) {
            if (!(size() == other.size())){
                throw new RuntimeException("must have same size");
            }
    ...
    public boolean isSame(ResourceBitMap other) {
        if (JVM.ENABLE_EXTRA_CHECK) {
            if (!(size() == other.size())){
                throw new RuntimeException("must have same size");
            }
